...
Cu.import('resource://gre/modules/L10nRegistryNoIndex.jsm');

const gInfoRequests = {
  resources: () => displayResources(L10nRegistry.requestResourceInfo()),
  cache: () => displayCache(L10nRegistry.requestCacheInfo()),
  resourcesNoIndex: () => displayResources(L10nRegistryNoIndex.requestResourceInfo(), true),
  cacheNoIndex: () => displayCache(L10nRegistryNoIndex.requestCacheInfo(), true),
};
...
function displayResources(data, noindex) {
  const cont = noindex ? 
    document.getElementById('resources_content-noindex') :
    document.getElementById('resources_content');
...
function displayCache(data, noindex) {
  const cont = noindex ? 
    document.getElementById('cache_content-noindex') : 
    document.getElementById('cache_content');
  const parent = cont.parentNode;
