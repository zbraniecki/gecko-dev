  ...
  *[Symbol.iterator]() {
  ...
  translateRoot(root, l10n) {
    return l10n.interactive.then(bundles => {
      ...
      performance.mark('translateRoot');
          ...
          for (let addedNode of mutation.addedNodes) {
            if (addedNode.nodeType === addedNode.ELEMENT_NODE) {
              if (addedNode.childElementCount) {
                const nodes = this.getTranslatables(addedNode);
  ...
  translateElements(elements) {
    performance.mark('translateElements');
  ...
  applyTranslations(elements, translations) {
    performance.mark('applyTranslationsStart');
    let l10n = this.get('main');
    this.pause();
    for (let i = 0; i < elements.length; i++) {
      l10n.overlayElement(elements[i], translations[i]);
    }
    this.resume();
    performance.mark('applyTranslationsEnd');
  }
  ...
  getKeysForElements(elems) {
    performance.mark('getKeysForElementsStart');
    let keys = [];
    for (let elem of elems) {

      //const id = 'new-navigator-key';
    ...
    performance.mark('getKeysForElementsEnd');
    return keys;
  }

  getElementsTranslation(elements) {
    performance.mark('getElementsTranslationStart');
    let l10n = this.get('main');
    let keys = this.getKeysForElements(elements);
    performance.mark('getElementsTranslationKeys');
    return l10n.formatEntities(keys);
  }
...
function fetchFirstBundle(bundles, createContext) {
  performance.mark('fetchFirstBundle');
...
function overlayElement(l10n, element, translation) {
  ...
  for (let key in translation.attrs) {
    if (l10n.isAttrAllowed({ name: key }, element)) {
      let val = translation.attrs[key] + ' -- ' + performance.now();
