...
static bool
RelativeTimeFormat(JSContext* cx, const CallArgs& args, bool construct)
{
    RootedObject obj(cx);
    ...
    if (construct) {
        RootedObject proto(cx, cx->global()->getOrCreateRelativeTimeFormatPrototype(cx));
        if (!proto)
            return false;
        obj = NewObjectWithGivenProto(cx, &RelativeTimeFormatClass, proto);
        if (!obj)
            return false;
...
{
    CallArgs args = CallArgsFromVp(argc, vp);
    return RelativeTimeFormat(cx, args, args.isConstructing());
}

bool
js::intl_RelativeTimeFormat(JSContext* cx, unsigned argc, Value* vp)
{
    CallArgs args = CallArgsFromVp(argc, vp);
    MOZ_ASSERT(args.length() == 2);
    // intl_RelativeTimeFormat is an intrinsic for self-hosted JavaScript, so it
    // cannot be used with "new", but it still has to be treated as a
    // constructor.
    return RelativeTimeFormat(cx, args, true);
}
...
InitRelativeTimeFormat(JSContext* cx, HandleObject Intl, Handle<GlobalObject*> global)
{
    ...
    RootedObject proto(cx, global->as<GlobalObject>().getOrCreateRelativeTimeFormatPrototype(cx));
    if (!proto)
        return nullptr;
    if (!LinkConstructorAndPrototype(cx, ctor, proto))
        return nullptr;
...
bool
GlobalObject::initRelativeTimeFormatProto(JSContext* cx, Handle<GlobalObject*> global)
{
    ...
    global->setReservedSlot(RELATIVE_TIME_FORMAT_PROTO, ObjectValue(*proto));
    return true;
}
